...
def get_adapter_ip(adapter_name):
    # Get the IP of the adapter with the specified name
    for interface, addrs in psutil.net_if_addrs().items():
        if interface == adapter_name:
            for addr in addrs:
                if addr.family == socket.AF_INET:  # IPv4 address
                    return addr.address
    raise ValueError(f"No IP address found for adapter '{adapter_name}'")
...
class SuearClient:
    ...
    def __init__(self, server=DEFAULT_SERVER, adapter_name="Wi-Fi 2", cmd_send_index=0):
        self.server = socket.gethostbyname(server)  # Server host name or IP address
        self.adapter_ip = get_adapter_ip(adapter_name)  # Dynamically retrieve adapter's IP
    ...
    def connect(self):
        ...
        if self.adapter_ip:
            self.command_sock.bind((self.adapter_ip, 0))  # Bind to specific adapter IP and an ephemeral port
    ...
    def open_video(self):
        ...
        # Initialize the stream connection with a socket bound to the second adapter’s IP
        stream_init_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        stream_init_sock.bind((self.adapter_ip, 0))  # Bind to adapter IP with an ephemeral port
        ...
        # Set up the main streaming socket, bound to the second adapter's IP
        self.stream_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.stream_sock.bind((self.adapter_ip, self.__class__.STREAM_RECV_PORT))  # Bind to specified adapter IP
        self.streaming = True

        # Set up a forwarding socket to send received frames to 127.0.0.1:45100
        self.forwarding_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.forward_address = ('127.0.0.1', 45100)  # Local forwarding address
        print(f"Forwarding stream to {self.forward_address}")
        ...
        while self.stream_sock is not None and not self.stream_sock._closed:
            ...
            while True:
                ...
                if parse_frame.complete:
                    ...
                    # Forward the complete frame to 127.0.0.1
                    self.forwarding_sock.sendto(frame.data, self.forward_address)
